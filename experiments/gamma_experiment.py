...
# gammas = [0.0, 1.0, 10.0, 50.0, 100.0, 250.0, 500.0, 1000.0, 5000.0]
gammas = [0, 100.0]
n_classes = 1
nrepetitions = 10
shots = [1, 5, 10]
...
experiment_logger = ExperimentLogger("gamma_experiment")
for dataset_name in dataset_names:
    ...
    models = [
        LAATLAATModel(
            model_name=f"laat_gpt-4o-mini_lr_{gamma}",
    ...
    models += [
        LAATLAATModel(
            model_name=f"laat_gpt-4o-mini_mlp_{gamma}",
    ...
    models += [
        LAATLAATModel(
            model_name=f"nesterov_laat_gpt-4o-mini_lr_{gamma}",
            model_class=partial_class(
                LAATUnweightedClassifier, module=TorchLogisticRegression, **nesterov_model_kwargs
            ),
            pandas_to_numpy_mapper=dataset.to_numpy,
            dataset=dataset,
            reasoning_llm=ChatOpenAI(model="gpt-4o-mini"),
    ...
    models += [
        LAATLAATModel(
            model_name=f"nesterov_laat_gpt-4o-mini_mlp_{gamma}",
            model_class=partial_class(LAATUnweightedClassifier, module=TorchMLP, **nesterov_model_kwargs),
            pandas_to_numpy_mapper=dataset.to_numpy,
            dataset=dataset,
            reasoning_llm=ChatOpenAI(model="gpt-4o-mini"),
            parsing_llm=ChatOpenAI(model="gpt-4o-mini", temperature=0.0),
